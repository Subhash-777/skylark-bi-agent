...
export default function AboutPage() {
  ...
  useEffect(() => {
    fetch('/api/chat', {
      ...
      body: JSON.stringify({
        messages: [{ role: 'user', content: 'What are the known data issues in our datasets?' }]
      })
    })
      .then(res => res.json())
      .then(() => {
        // Fallback default list if chat endpoint response is formatted
        setIssues([
          {
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg-primary)', color: 'var(--text-primary)', display: 'flex', flexDirection: 'column' }}>
      ...
      <main style={{ flex: 1, padding: '40px 24px', maxWidth: '1100px', margin: '0 auto', width: '100%' }}>
        ...
        <div style={{ marginBottom: '40px' }}>
          <div style={{ fontSize: '13px', fontWeight: 600, color: 'var(--text-accent)', marginBottom: '8px' }}>TECHNICAL ARCHITECTURE & DECISION LOG</div>
        ...
        <section style={{
          ...
          <h2 style={{ fontSize: '18px', fontWeight: 700, marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '8px' }}>
            <span>🛠️</span> The Tool-Calling Agent Loop
          </h2>

          <div style={{ background: 'var(--bg-primary)', padding: '20px', borderRadius: '12px', fontFamily: 'var(--font-mono)', fontSize: '13px', lineHeight: 1.7, color: 'var(--text-secondary)', overflowX: 'auto', border: '1px solid var(--border)' }}>
            <div style={{ color: 'var(--text-accent)' }}>User Question (&quot;How is Mining pipeline looking?&quot;)</div>
            <div>└─► <strong>1. Gemini 2.5 Flash Agent Loop</strong></div>
            <div>    ├─► Calls <span style={{ color: 'var(--info)' }}>get_schema()</span> → inspects clean table definitions &amp; sync completeness %</div>
            <div>    ├─► Calls <span style={{ color: 'var(--info)' }}>run_query(sql)</span> → executes read-only SELECT against Postgres mirror</div>
            <div>    ├─► Calls <span style={{ color: 'var(--info)' }}>list_known_data_issues()</span> → checks null caveats for queried fields</div>
            <div>    └─► <strong>2. Synthesizes Final Answer</strong></div>
            <div style={{ color: 'var(--success)', marginTop: '4px' }}>        └─► Returns Answer + Interactive Recharts + SQL Audit Trail + Coverage Footnote</div>
          </div>
        </section>
        ...
        <section style={{ marginBottom: '40px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '20px' }}>
            <div className="digest-card" style={{ borderLeft: '4px solid var(--danger)' }}>
              <h3 style={{ fontSize: '15px', fontWeight: 600, color: 'var(--danger)', marginBottom: '8px' }}>❌ The Naive Approach (Prompt-Stuffing)</h3>
            ...
            <div className="digest-card" style={{ borderLeft: '4px solid var(--success)' }}>
              <h3 style={{ fontSize: '15px', fontWeight: 600, color: 'var(--success)', marginBottom: '8px' }}>✅ Our Approach (Postgres SQL Code Execution)</h3>
              <p style={{ fontSize: '13px', color: 'var(--text-secondary)', lineHeight: 1.6 }}>
        ...
        <section style={{ marginBottom: '40px' }}>
          ...
          <p style={{ fontSize: '14px', color: 'var(--text-secondary)', marginBottom: '20px' }}>
            The source CSV exports contained specific structural anomalies. Here is how our cleaning pipeline (<code style={{ color: 'var(--info)' }}>lib/sync.ts</code>) handles each:
          </p>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
              <thead>
                <tr style={{ background: 'var(--bg-tertiary)' }}>
                  <th style={{ padding: '12px', textAlign: 'left', color: 'var(--text-accent)', border: '1px solid var(--border)' }}>Data Bug Found</th>
                  <th style={{ padding: '12px', textAlign: 'left', color: 'var(--text-accent)', border: '1px solid var(--border)' }}>Impacted Columns</th>
                  <th style={{ padding: '12px', textAlign: 'left', color: 'var(--text-accent)', border: '1px solid var(--border)' }}>Handling &amp; Mitigation Strategy</th>
                </tr>
              </thead>
              <tbody>
                {issues.map((issue, idx) => (
                  <tr key={idx}>
                    <td style={{ padding: '12px', border: '1px solid var(--border)', fontWeight: 600 }}>{issue.id.replace(/_/g, ' ')}</td>
                    <td style={{ padding: '12px', border: '1px solid var(--border)', fontFamily: 'var(--font-mono)', fontSize: '12px', color: 'var(--info)' }}>
                      {issue.affected_tables.join(', ')}
                    </td>
                    <td style={{ padding: '12px', border: '1px solid var(--border)', color: 'var(--text-secondary)' }}>
                      {issue.handling}
                    </td>
                  </tr>
                ))}
        ...
        {/* Quick Links */}
        <section style={{ textAlign: 'center', padding: '32px', background: 'var(--bg-card)', borderRadius: '16px', border: '1px solid var(--border)' }}>
          <h3 style={{ fontSize: '18px', fontWeight: 700, marginBottom: '12px' }}>Ready to test the agent?</h3>
          <p style={{ fontSize: '14px', color: 'var(--text-secondary)', marginBottom: '20px' }}>Try running natural language queries or explore the executive digest.</p>
